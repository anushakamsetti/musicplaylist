...
class MusicPlayList extends Component {
  ...
  onClickDeleteTrack = id => {
    const {palyList} = this.state
    const updatedPlayList = palyList.filter(each => each.id !== id)
    this.setState({palyList: updatedPlayList})
  }
  ...
  render() {
    const {palyList, searchInput} = this.state
    const searchResults = palyList.filter(eachTrack =>
